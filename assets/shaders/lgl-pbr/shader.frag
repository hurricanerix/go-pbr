...
void main()
{
    ...
    lightPositions[1] = lightPos;
    lightPositions[2] = lightPos;
    lightPositions[3] = lightPos;
    vec3 lightColors[4];
    lightColors[0] = LightColor;
    lightColors[1] = LightColor;
    lightColors[2] = LightColor;
    lightColors[3] = LightColor;

    vec3 camPos = viewPos;

    vec3 albedo     = pow(texture(albedoMap, TexCoords).rgb, vec3(2.2));
    float metallic  = texture(armMap, TexCoords).r;
    float roughness = texture(armMap, TexCoords).g;
    float ao        = texture(armMap, TexCoords).b;
